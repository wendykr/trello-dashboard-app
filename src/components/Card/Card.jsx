...
export const Card = ({
  ...
  const filteredComments = comments.filter(
    (oneComment) => oneComment.cardId === id,
  );
  ...
  const titleDateEnd = done
    ? "Karta je hotová"
    : dayjs(dateEnd).isAfter(dayjs())
      ? "Karta má termín později"
      : dayjs(dateEnd).isSame(dayjs(), "day")
        ? "Karta má dnešní termín"
        : "Karta je po termínu";

  const formatDateTime =
    dateEnd !== "" && dateStart === ""
      ? dayjs(dateEnd).format(
          `DD.MM.${dayjs(dateEnd).format("YYYY") !== dayjs().format("YYYY") ? "YYYY" : ""}`,
        )
      : dateEnd !== "" && dateStart !== ""
    ...
    >
      {src && (
        <figure className="flex items-center">
          <img
            className="sm:max-h-36 max-h-52 w-full rounded-t-lg"
            src={src}
            alt="***"
          />
        </figure>
      ...
      <div className="p-2">
        ...
        <p className="outline-none text-[#172b4d]">{text}</p>
        {(dateStart !== "" ||
          dateEnd !== "" ||
          description ||
          ...
          <aside className="mb-0.5 mt-1.5 flex items-center gap-2">
            {((dateEnd !== "" && dateStart === "") ||
              (dateEnd !== "" && dateStart !== "")) && (
              <span
                className={`px-1 py-0.5 ${styledDateEnd} rounded-[3px] flex items-center gap-1 hover:brightness-[85%] hover:saturate-[85%]`}
                title={titleDateEnd}
              ...
              >
                {showCheckIcon ? (
                    ...
                    <svg
                      className={`w-[18px] h-[18px] ${dayjs(dateEnd).isAfter(dayjs()) ? "text-[#172b4d]" : "text-white"}`}
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      strokeWidth="1.5"
            ...
            {dateEnd === "" && dateStart !== "" && (
              <span
                className={`px-1 py-0.5 rounded-[3px] flex items-center gap-1`}
                onClick={handleClickDone}
                onMouseEnter={handleMouseEnter}
                onMouseLeave={handleMouseLeave}
              >
                ...
                <span className="text-[12px]">
                  ...
                  {dayjs(dateStart).format(
                    `DD.MM.${dayjs(dateStart).format("YYYY") !== dayjs().format("YYYY") ? "YYYY" : ""}`,
                  )}
                </span>
              </span>
            )}
